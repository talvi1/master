...
*/
void uart_send_message(uint8_t *v)
{
	uint8_t data_t[1];
	data_t[0] = *v;
	
	HAL_UART_Transmit(&huart2,(uint8_t *)data_t, 8, 10000);
	
}

uint8_t uart_receive_message(uint8_t *v){
	uint8_t data_t[1];
	data_t[0] = *v;
	
	return HAL_UART_Receive(&huart2,(uint8_t *)data_t, 11, 10000);}
/*
Function: print_str
